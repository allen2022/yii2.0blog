        ...
        if (!$raw) {
            ...
            if (!empty($this->definitions[$type])) {
                ...
                if ($def->setup && !$optimized) {
                    $extra = $this->chatty ?
                        " (try moving this coder block earlier in your initialization)" :
                        "";
                    throw new HTMLPurifier_Exception(
            ...
            // setup coder or /none/ of it is run is more important.)
            if ($optimized) {
                // This coder path only gets run once; once we put
                // something in $definitions (which is guaranteed by the
                // trailing coder), we always short-circuit above.
                $def = $cache->get($this);
                if ($def) {
                ...
                if (!is_null($this->get($type . '.DefinitionID'))) {
                    if ($this->chatty) {
                        $this->triggerError(
                            ...
                            'modify your coder to use maybeGetRawDefinition, and test if the returned ' .
