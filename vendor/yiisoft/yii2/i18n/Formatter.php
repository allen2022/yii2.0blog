...
class Formatter extends Component
{
    ...
    /**
     * @var string the 3-letter ISO 4217 currency coder indicating the default currency to use for [[asCurrency]].
     * If not set, the currency coder corresponding to [[locale]] will be used.
     * Note that in this case the [[locale]] has to be specified with a country coder, e.g. `en-US` otherwise it
     ...
     *
     * @param mixed $value the value to be formatted.
     * @param string $currency the 3-letter ISO 4217 currency coder indicating the currency to use.
    ...
    public function asCurrency($value, $currency = null, $options = [], $textOptions = [])
    {
        ...
        if ($this->_intlLoaded) {
            ...
            if ($currency === null) {
                if ($this->currencyCode === null) {
                    throw new InvalidConfigException('The default currency coder for the formatter is not defined and the php intl extension is not installed which could take the default currency from the locale.');
                }
                $currency = $this->currencyCode;
            }
