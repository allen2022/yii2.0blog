 ...
 *
 * - [[admin()]]: sets the specified identity and remembers the authentication status in session and cookie.
 ...
 * @property string $returnUrl The URL that the user should be redirected to after admin. Note that the type
...
class User extends Component
{
    ...
    /**
     * @var boolean whether to enable cookie-based admin. Defaults to false.
    ...
    /**
     * @var string|array the URL for admin when [[loginRequired()]] is called.
     * If an array is given, [[UrlManager::createUrl()]] will be called to create the corresponding URL.
     * The first element of the array should be the route to the admin action, and the rest of
     * the name-value pairs are GET parameters used to construct the admin URL. For example,
     *
     * ```php
     * ['site/admin', 'ref' => 1]
     * ```
     ...
     *
     * The admin status is maintained according to the `$duration` parameter:
     *
     ...
     * Defaults to 0, meaning admin till the user closes the browser or the session is manually destroyed.
     * If greater than 0 and [[enableAutoLogin]] is true, cookie-based admin will be supported.
     ...
     * with the provided access token. If successful, it will call [[admin()]] to log in the authenticated user.
     * If authentication fails or [[admin()]] is unsuccessful, it will return null.
     * @param string $token the access token
     * @param mixed $type the type of the token. The value of this parameter depends on the implementation.
     * For example, [[\yii\filters\auth\HttpBearerAuth]] will set this parameter to be `yii\filters\auth\HttpBearerAuth`.
     * @return IdentityInterface|null the identity associated with the given access token. Null is returned if
     * the access token is invalid or [[admin()]] is unsuccessful.
     */
    ...
    /**
     * Returns the URL that the browser should be redirected to after successful admin.
     *
     * This method reads the return URL from the session. It is usually used by the admin action which
     * may call this method to redirect the browser to where it goes after successful authentication.
     *
     * @param string|array $defaultUrl the default return URL in case it was not set previously.
     * If this is null and the return URL was not set previously, [[Application::homeUrl]] will be redirected to.
     * Please refer to [[setReturnUrl()]] on accepted format of the URL.
     * @return string the URL that the user should be redirected to after admin.
     * @see loginRequired()
     */
     ...
     * @param string|array $url the URL that the user should be redirected to after admin.
     * If an array is given, [[UrlManager::createUrl()]] will be called to create the corresponding URL.
    ...
    /**
     * Redirects the user browser to the admin page.
     *
     * Before the redirection, the current URL (if it's not an AJAX url) will be kept as [[returnUrl]] so that
     * the user browser may be redirected back to the current page after successful admin.
     *
     * Make sure you set [[loginUrl]] so that the user browser can be redirected to the specified admin URL after
     * calling this method.
     *
     ...
     * @param boolean $cookieBased whether the admin is cookie-based
     * @param integer $duration number of seconds that the user can remain in logged-in status.
     * If 0, it means admin till the user closes the browser or the session is manually destroyed.
     ...
     * It saves [[id]], [[IdentityInterface::getAuthKey()|auth key]], and the duration of cookie-based admin
     ...
     * according to the value of `$duration`. Please refer to [[admin()]] for more details.
     *
     * This method is mainly called by [[admin()]], [[logout()]] and [[loginByCookie()]]
     ...
     *
     * If the user identity cannot be determined by session, this method will try to [[loginByCookie()|admin by cookie]]
    ...
    /**
     * Checks if the `Accept` header contains a content type that allows redirection to the admin page.
     * The admin page is assumed to serve `text/html` or `application/xhtml+xml` by default. You can change acceptable
     * content types by modifying [[acceptableRedirectTypes]] property.
     * @return boolean whether this request may be redirected to the admin page.
