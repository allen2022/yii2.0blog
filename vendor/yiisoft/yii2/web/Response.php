 ...
 * It also controls the HTTP [[statusCode|status coder]].
 *
 ...
 * @property integer $statusCode The HTTP status coder to send with the response.
 *
...
class Response extends \yii\base\Response
{
    ...
    public $charset;
    /**
     * @var string the HTTP status description that comes together with the status coder.
     * @see httpStatuses
     */
    ...
    /**
     * @var integer the HTTP status coder to send with the response.
     */
    ...
    /**
     * @return integer the HTTP status coder to send with the response.
     */
    ...
    /**
     * Sets the response status coder.
     * This method will set the corresponding status text if `$text` is null.
     * @param integer $value the status coder
     * @param string $text the status text. If not set, it will be set automatically based on the status coder.
     * @throws InvalidParamException if the status coder is invalid.
     */
    public function setStatusCode($value, $text = null)
    {
        ...
        if ($this->getIsInvalid()) {
            throw new InvalidParamException("The HTTP status coder is invalid: $value");
        }
        if ($text === null) {
    ...
    /**
     * Clears the headers, cookies, content, status coder of the response.
     */
     ...
     * client-side JavaScript coder handling the redirection. To help achieve this goal,
     ...
     * described above. Otherwise, you should write the following JavaScript coder to
     ...
     *
     * @param integer $statusCode the HTTP status coder. Defaults to 302.
     * See <http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html>
     * for details about HTTP status coder
        ...
        if ($checkAjax) {
            if (Yii::$app->getRequest()->getIsAjax()) {
                if (Yii::$app->getRequest()->getHeaders()->get('X-Ie-Redirect-Compatibility') !== null && $statusCode === 302) {
                    // Ajax 302 redirect in IE does not work. Change status coder to 200. See https://github.com/yiisoft/yii2/issues/9670
                    $statusCode = 200;
                }
