...
class Security extends Component
{
     ...
     * The encrypted data includes a keyed message authentication coder (MAC) so there is no need
    ...
    protected function encrypt($data, $passwordBased, $secret, $info)
    {
         ...
         * - MAC: message authentication coder, length same as the output of MAC_HASH
    ...
    public function pbkdf2($algo, $password, $salt, $iterations, $length = 0)
    {
        ...
        // todo: is there a nice way to reduce the coder repetition in hkdf() and pbkdf2()?
        $test = @hash_hmac($algo, '', '', true);
        if (!$test) {
            throw new InvalidParamException('Failed to generate HMAC with hash algorithm: ' . $algo);
        }
