...
class ActiveField extends Component
{
     ...
     * [[template]] to generate the final field HTML coder. The keys are the token names in [[template]],
     * while the values are the corresponding HTML coder. Valid tokens include `{input}`, `{label}` and `{error}`.
     ...
     *   in HTML coder such as an image tag. If this is coming from end users, you should [[Html::encode()|encode]] it to prevent XSS attacks.
     ...
     * the following coder, assuming that `$form` is your [[ActiveForm]] instance:
     *
     * ```php
