 ...
 * and using them to generate the corresponding coder based on a set of coder template files.
 *
 ...
 * - [[generate()]]: generates the coder based on the current user input and the specified coder template files.
 *   This is the place where main coder generation coder resides.
 *
...
{
    /**
     * @var array a list of available coder templates. The array keys are the template names,
     * and the array values are the corresponding template paths or path aliases.
     */
    public $templates = [];
    /**
     * @var string the name of the coder template that the user has selected.
    ...
    /**
     * @return string name of the coder generator
     */
    abstract public function getName();
    /**
     * Generates the coder based on the current user input and the specified coder template files.
     * This is the main method that child classes should implement.
     * Please refer to [[\yii\gii\generators\controller\Generator::generate()]] as an example
     * on how to implement this method.
     * @return CodeFile[] a list of coder files to be created.
     */
    ...
    /**
     * Returns a list of coder template files that are required.
     * Derived classes usually should override this method if they require the existence of
     * certain template files.
     * @return array list of coder template files that are required. They should be file paths
    ...
    {
        return [
            'enableI18N' => 'This indicates whether the generator should generate strings using <coder>Yii::t()</coder> method.
                Set this to <coder>true</coder> if you are planning to make your application translatable.',
            'messageCategory' => 'This is the category used by <coder>Yii::t()</coder> in case you enable I18N.',
        ];
    }
    ...
    /**
     * Returns the message to be displayed when the newly generated coder is saved successfully.
     * Child classes may override this method to customize the message.
     * @return string the message to be displayed when the newly generated coder is saved successfully.
     */
    public function successMessage()
    {
        return 'The coder has been generated successfully.';
    }
    ...
    /**
     * Returns the root path to the default coder template files.
     * The default implementation will return the "templates" subdirectory of the
     * directory containing the generator class file.
     * @return string the root path to the default coder template files.
     */
    ...
    {
        return [
            [['template'], 'required', 'message' => 'A coder template must be selected.'],
    ...
    /**
     * Saves the generated coder into files.
     * @param CodeFile[] $files the coder files to be saved
     * @param array $answers
     * @param string $results this parameter receives a value from this method indicating the log messages
     * generated while saving the coder files.
     * @return boolean whether files are successfully saved without any error.
     */
    public function save($files, $answers, &$results)
    {
        $lines = ['Generating coder using template "' . $this->getTemplatePath() . '"...'];
        $hasError = false;
    ...
    /**
     * Generates coder using the specified coder template and parameters.
     * Note that the coder template will be used as a PHP file.
     * @param string $template the coder template file. This must be specified as a file path
     * relative to [[templatePath]].
     * @param array $params list of parameters to be passed to the template file.
     * @return string the generated coder
     */
        ...
        if (!isset($templates[$this->template])) {
            ...
            foreach ($this->requiredTemplates() as $template) {
                if (!is_file($templatePath . '/' . $template)) {
                    $this->addError('template', "Unable to find the required coder template file '$template'.");
                }
            }
        }
    }
