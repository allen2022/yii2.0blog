yii.gii = (function ($) {
    ...
    var initPreviewDiffLinks = function () {
        $('.preview-coder, .diff-coder, .modal-refresh, .modal-previous, .modal-next').on('click', function () {
            var $modal = $('#preview-modal');
            var $link = $(this);
            $modal.find('.modal-refresh').attr('href', $link.attr('href'));
            if ($link.hasClass('preview-coder') || $link.hasClass('diff-coder')) {
                $modal.data('action', ($link.hasClass('preview-coder') ? 'preview-coder' : 'diff-coder'))
            }
